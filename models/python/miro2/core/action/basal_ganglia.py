...
class BasalGanglia(object):
	...
	def __init__(self, state, pars, channels):
	...
	def update(self, node):
		...
		# # Pass inputs to BGR model
		# FIXME: Inputs are currently too low to trigger BG selection
		self.step(self.prio)

		# Select action with highest MCx output
		# TODO: Revise this so only actions >0.95 are selected
		selected = np.argmax(self.pop['Ventral']['Ctx']['o'])
		...
		if self.selected != selected:
			...
			# print('Inputs: ' + str(self.prio))
			# print('Outputs: ' + str(self.pop['Ventral']['Ctx']['o']))
